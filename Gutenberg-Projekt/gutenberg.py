...
s = urllib.request.urlopen(base + "/index.html")
# s = urllib.urlopen(base + "/index.html")
# t = s.read()
t = s.read().decode('utf-8')  # oder die passende Zeichenkodierung verwenden
...
while j < l:
    ...
    b = t.decode('utf-8').find('Inhalt')
    b2 = b

    while True:
        b2 = t.decode('utf-8').find("<p>", b)+3
        if b2 < 3:
            break
        b = t.decode('utf-8').find("</p>", b+1)
        if b < 0:
            break
        f.write(paragraphS + t[b2:b].decode('utf-8', errors='ignore') + paragraphE)
        #f.write(paragraphS + t[b2:b].decode('utf-8') + paragraphE)
        f.flush()
...
if format == "pdf":
    os.system("python3 convert.py '" + name + "'")
